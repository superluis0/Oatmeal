...
- 🛟 **Recordings are safe:** the audio is saved the moment you stop, so a hiccup (LM Studio offline, an interrupted run) never loses a meeting — re-run the transcript and summary from the recording in one tap.
